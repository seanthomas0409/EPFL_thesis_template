...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|}
        ...
        $\frac{e}{r} \uparrow$ & $\Delta\theta_\text{adm} \downarrow \;  \Rightarrow \mathbb{X}_\text{el} \downarrow$  & \multirow{3}{*}{$K_\theta \uparrow \;
        \Rightarrow \begin{cases} \mathbb{X}_\text{eq} \uparrow\\ F_\text{grip} \downarrow\\
        \end{cases}$}     \\\cline{1-2}

        $e \uparrow \;|\; \frac{e}{r}$ fixed & $S_{\text{mech}}\uparrow $ & \\
        \cline{1-2}

        $b \uparrow$ & $t_{\text{mech}}\uparrow $ &\\
        \hline

        \arrayrulecolor{black}
         $x_{\text{off}}\uparrow$ & \multicolumn{2}{c|}{$\mathbb{X}_\text{eq}\uparrow$,\quad $\mathbb{X}_\text{el}$ shifted away from $\mathbb{X}_\text{eq}$}\\
         \hline
    \end{tabular}
\end{table}
