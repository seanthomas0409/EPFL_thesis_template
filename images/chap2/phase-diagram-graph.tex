...
\begin{tikzpicture}
    ...
    \node[anchor=center] (twtodetw) at (1.4,2.5) {\tiny $M^\mathrm{t}\rightarrow M^\mathrm{d}$};

    \node[anchor=center] (Atext) at (7.25,1.25) {\tiny Austenite};

    \coordinate (M2As) at ($(As)+(1.5,3)$);
    \draw[->,>=latex] (M2As) -- node[above,sloped,scale=0.85,yshift=2] {\tiny $M^\mathrm{d}\rightarrow A$} ($(Af)!(M2As)!(Af_)$);

    \coordinate (A2Ms) at ($(Ms)+(1.75,3.5)$);
    \draw[->,>=latex] (A2Ms) -- node[above,sloped,scale=0.85,yshift=2] {\tiny $M^\mathrm{d}\leftarrow A$} ($(Mf)!(A2Ms)!(Mf_)$);
\end{tikzpicture}
\end{document}
