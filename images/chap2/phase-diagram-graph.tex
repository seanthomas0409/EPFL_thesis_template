...
% }
\newcommand{\tick}{\tikz \draw[-,thick] (0,0) -- +(0,0.1);}
% \newcommand{\tick}{\tikz \draw[-,thick] (0,-0.2) -- +(0,0.2);}
\newcommand{\tickH}{\tikz \draw[-,thick] (0,0) -- +(0.1,0);}
...
\begin{tikzpicture}
    ...
    \draw[-] (0,0) -- (8.5,0) node[pos=0.5   ,below, yshift=-17]{\textbf{\footnotesize Temperature $T$ [$^\circ$C]}} -- (8.5,6.0);
    \draw[-] (0,0) -- (0,6.0) node[pos=0.5,above,rotate=90, yshift=20] {\textbf{\footnotesize Stress $\sigma$ [Pa]}} -- (8.5,6.0);
    ...
    \node[label=below:\scriptsize$M_\mathrm{f}$] (Mftext) at (2,0) {\tick};
    \coordinate (Mf_) at ($(Mf)+(3,6)$);
    \coordinate (Ms) at (3,0);
    \node[label=below:\scriptsize$M_\mathrm{s}$] (Mstext) at (3,0) {\tick};
    \coordinate (Ms_) at ($(Ms)+(3,6)$) {};
    \coordinate (As) at (4,0);
    \node[label=below:\scriptsize$A_\mathrm{s}$] (Astext) at (4,0) {\tick};
    \coordinate (As_) at ($(As)+(3,6)$) {};
    \coordinate (Af) at (5,0);
    \node[label=below:\scriptsize$A_\mathrm{f}$] (Aftext) at (5,0) {\tick};
    ...
    \node[label=left:\scriptsize$\sigma_\mathrm{s}^\mathrm{cr}$] (sstext) at (0,2) {\tickH};
    \coordinate (sf) at (0,3);
    \node[label=left:\scriptsize$\sigma_\mathrm{f}^\mathrm{cr}$] (sftext) at (0,3) {\tickH};
